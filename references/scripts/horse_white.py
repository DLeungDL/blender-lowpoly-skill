"""White Horse detailed reconstruction from White Horse.glb (glb2).

Source: /workspace/refs/glb2/White Horse.glb (detailed).
Measured: 4400 verts, 2182 tris, 7 slots (Main, Hair, Muzzle, Hooves,
Main_Light, Eye_Black, Eye_White), 50 bones, 26 actions. Same topology as
the brown detailed Horse without Main_Dark.

Primitive approximation, not a vertex clone.
"""
...
COL_NAME = 'REF_WhiteHorse_Detailed'
MESH_NAME = 'WhiteHorse'
...
def build():
    ...
    main_mat = flat_mat('Main', (0.3543, 0.3584, 0.2911, 1.0000))
    hair_mat = flat_mat('Hair', (0.3288, 0.2799, 0.1968, 1.0000))
    muz_mat = flat_mat('Muzzle', (0.2487, 0.1618, 0.0977, 1.0000))
    hoof_mat = flat_mat('Hooves', (0.2306, 0.1825, 0.0665, 1.0000))
    light_mat = flat_mat('Main_Light', (0.2306, 0.2212, 0.2283, 1.0000))
    eye_b_mat = flat_mat('Eye_Black', (0.0072, 0.0072, 0.0072, 1.0000))
    eye_w_mat = flat_mat('Eye_White', (0.3895, 0.3895, 0.3895, 1.0000))
    parts = []

    # White Horse: same topology as detailed Horse, 7 slots (no Main_Dark)
    ...
    parts = [body, chest, rump, neck, head, blaze, muzzle, ebl, ebr, ewl, ewr,
